...
typedef struct __http_tuple
{
    int http_header; // enum like Host, Accept etc
    char http_header_value[128]; // name like ya.ru, text/html;level=1
} __http_tuple_t;
typedef __http_tuple_t* http_tuple_t;

http_tuple_t init_tuple(int key, const char value[128]);
void destroy_tuple(http_tuple_t);
// TODO Linked_list
typedef struct __headers_tuples
{
    http_tuple_t headers[HEADERS_LIMIT];
    size_t headers_size;
    http_header_type_t type;
}http_headers_tuples_array_t;

typedef struct  __headers
{
    http_headers_tuples_array_t general_headers;
    http_headers_tuples_array_t request_headers;
    http_headers_tuples_array_t response_headers;
    http_headers_tuples_array_t entity_headers;

}http_conn_headers_t;
http_conn_headers_t* init_headers(void);
